...
private:
	...
	{
		if (ack.isEmpty())
			return QICResult<>::CreateFailedResult(u8"接收的数据长度为0");
		// ASCII for 'E'
		if (ack.at(0) == 69)
			return QICResult<>::CreateFailedResult(u8"PLC反馈信号错误：" + QString::fromStdString(ack.toStdString()));
		// ASCII for '\n' and '\r'
		if (ack.at(ack.size() - 1) != 10 || ack.at(ack.size() - 2) != 13)
			return QICResult<>::CreateFailedResult(u8"PLC反馈信号错误：" + ack.toHex(' '));
