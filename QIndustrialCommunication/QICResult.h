...
public:
	...
	//// 重载版本：当Index为0时，简化获取第一个Content元素
	//auto getContent() const
	//{
	//	return std::get<0>(Content);
	//}

	//// 重载版本：当Index为0时，简化设置第一个Content元素
	//template <typename U>
	//void setContent(U&& value)
	//{
	//	using ElementType = std::tuple_element_t<0, std::tuple<T...>>;
	//	static_assert(std::is_same_v<ElementType, std::decay_t<U>>, "Type mismatch: Provided value type does not match the Content element type.");
	//	std::get<0>(Content) = std::forward<U>(value);
	//}
