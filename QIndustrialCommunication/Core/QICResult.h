...
#pragma region Generate getContent setContent convenient method
	...
	// 注意：下面原始写法在MSVC下可以编译，但Clang会在类实例化阶段提前求值enable_if<false>并报错，故改为依赖局部参数的写法
	// template<typename = typename std::enable_if_t<has_index<0>::value>> auto getContent0() const { return getContentN<0>(); }
	// template<typename = typename std::enable_if_t<has_index<1>::value>> auto getContent1() const { return getContentN<1>(); }
	// template<typename = typename std::enable_if_t<has_index<2>::value>> auto getContent2() const { return getContentN<2>(); }
	// template<typename = typename std::enable_if_t<has_index<3>::value>> auto getContent3() const { return getContentN<3>(); }
	// template<typename = typename std::enable_if_t<has_index<4>::value>> auto getContent4() const { return getContentN<4>(); }
	// template<typename = typename std::enable_if_t<has_index<5>::value>> auto getContent5() const { return getContentN<5>(); }
	template <typename _D = void, typename std::enable_if<has_index<0>::value && std::is_void<_D>::value, int>::type = 0>
	auto getContent0() const { return getContentN<0>(); }
	template <typename _D = void, typename std::enable_if<has_index<1>::value && std::is_void<_D>::value, int>::type = 0>
	auto getContent1() const { return getContentN<1>(); }
	template <typename _D = void, typename std::enable_if<has_index<2>::value && std::is_void<_D>::value, int>::type = 0>
	auto getContent2() const { return getContentN<2>(); }
	template <typename _D = void, typename std::enable_if<has_index<3>::value && std::is_void<_D>::value, int>::type = 0>
	auto getContent3() const { return getContentN<3>(); }
	template <typename _D = void, typename std::enable_if<has_index<4>::value && std::is_void<_D>::value, int>::type = 0>
	auto getContent4() const { return getContentN<4>(); }
	template <typename _D = void, typename std::enable_if<has_index<5>::value && std::is_void<_D>::value, int>::type = 0>
	auto getContent5() const { return getContentN<5>(); }

	// 便捷setContent方法 - 使用SFINAE(Substitution Failure Is Not An Error)确保方法只在对应索引存在时可用
	// 注意：下面原始写法在MSVC下可以编译，但Clang会在类实例化阶段提前求值enable_if<false>并报错，故改为依赖局部参数的写法
	// template<typename U, typename = typename std::enable_if_t<has_index<0>::value>> void setContent0(U&& value) { setContentN<0>(std::forward<U>(value)); }
	// template<typename U, typename = typename std::enable_if_t<has_index<1>::value>> void setContent1(U&& value) { setContentN<1>(std::forward<U>(value)); }
	// template<typename U, typename = typename std::enable_if_t<has_index<2>::value>> void setContent2(U&& value) { setContentN<2>(std::forward<U>(value)); }
	// template<typename U, typename = typename std::enable_if_t<has_index<3>::value>> void setContent3(U&& value) { setContentN<3>(std::forward<U>(value)); }
	// template<typename U, typename = typename std::enable_if_t<has_index<4>::value>> void setContent4(U&& value) { setContentN<4>(std::forward<U>(value)); }
	// template<typename U, typename = typename std::enable_if_t<has_index<5>::value>> void setContent5(U&& value) { setContentN<5>(std::forward<U>(value)); }
	template <typename U, typename _D = void, typename std::enable_if<has_index<0>::value && std::is_void<_D>::value, int>::type = 0>
	void setContent0(U &&value) { setContentN<0>(std::forward<U>(value)); }
	template <typename U, typename _D = void, typename std::enable_if<has_index<1>::value && std::is_void<_D>::value, int>::type = 0>
	void setContent1(U &&value) { setContentN<1>(std::forward<U>(value)); }
	template <typename U, typename _D = void, typename std::enable_if<has_index<2>::value && std::is_void<_D>::value, int>::type = 0>
	void setContent2(U &&value) { setContentN<2>(std::forward<U>(value)); }
	template <typename U, typename _D = void, typename std::enable_if<has_index<3>::value && std::is_void<_D>::value, int>::type = 0>
	void setContent3(U &&value) { setContentN<3>(std::forward<U>(value)); }
	template <typename U, typename _D = void, typename std::enable_if<has_index<4>::value && std::is_void<_D>::value, int>::type = 0>
	void setContent4(U &&value) { setContentN<4>(std::forward<U>(value)); }
	template <typename U, typename _D = void, typename std::enable_if<has_index<5>::value && std::is_void<_D>::value, int>::type = 0>
